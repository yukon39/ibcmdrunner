...
//
Процедура ПараметрыСервера(Порт = 8314,
	Адрес = "localhost",
...
//
// Параметры:
//   ИспользоватьШлюзПрямогоДоступа - Булево - использовать доступ по протоколу TCP/IP (прямое подключение).
//   ИспользоватьШлюзHTTP - Булево - использовать доступ к автономному серверу по протоколу HTTP.
//   ИспользоватьШлюзSSH - Булево - использовать доступ к автономному серверу по протоколу SSH.
//   ИспользоватьРасширеннуюФункциональностьКонифгуратора - Булево - использовать расширенную 
//		функциональность Конфигуратора.
//
Процедура ПараметрыФункциональностиСервера(ИспользоватьШлюзПрямогоДоступа = Истина,
		ИспользоватьШлюзHTTP = Истина,
		ИспользоватьШлюзSSH = Истина,
		ИспользоватьРасширеннуюФункциональностьКонифгуратора = Ложь) Экспорт
...
//
// Параметры:
//   ТипОтладки - Строка - Режим работы отладчика. (tcp|http)
...
Процедура ЗаписатьПараметрыВебДоступа(Конфиг)
	...
	Если ПараметрыХТТП.Количество() = 0 
